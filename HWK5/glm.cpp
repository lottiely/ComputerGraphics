...
Additions by Lazaros Karydas
Tested under Windows and Linux

v-0.8
- Added support for groups of vertices and materials
- Each model has each own VAO and each group of vertices each own VBO
...
glmReadMTL(GLMmodel* model, char* name)
{
  ...
  for (i = 0; i < nummaterials; i++) {
    ...
    model->materials[i].transparency = 1.0;
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'N':
      ...
      break;
    case 'd':
      fscanf(file, "%f", &model->materials[nummaterials].transparency);
      break;
    case 'K':
      switch(buf[1]) {
      case 'd':
...
GLvoid glmLoadInVBO(GLMmodel* model)
{
  ...
  while (group) 
    {
      GLuint numPointsInVBO = 3 * group->numtriangles;
      if (numPointsInVBO == 0)
	{
	  group = group->next;
	  continue;
...
glmDrawVBO(GLMmodel* model, GLuint program)
{
  ...
  while(group)
      ...
      if (material->transparency < 1.0)
	{
      ...
      glUniform4fv( glGetUniformLocation(program, "mat_ambient"),1, material->ambient );
      glUniform4fv( glGetUniformLocation(program, "mat_diffuse"),1,material->diffuse );
