...
//
//Problem 1 
//	 This code outputs an octogonal space where a the brownian motion is randomly placed inside the space.
//	 The brownian motion will continue to move around the space until it collides with the bounds of the space.
//   The start of the motion is shown by a green marker.
//	 The end of the motion is shown by a red marker, and the wall the fly collides into will become opaque.
//
//Interaction:
//	 Keyboard:
//		x:			will move the space counter clockwise on the x-axis
//		shift+x:	will move the space clockwise on the x-axis
//		y:			will move the space counter clockwise on the y-axis
//		shift+y:	will move the space clockwise on the y-axis
//		z:			will move the space counter clockwise on the z-axis
//		shift+z:	will move the space clockwise on the z-axis
//
//	 Mouse:
...
// Vertices of a unit octogonal space centered at origin, sides aligned with axes
point4 vertices[16] = {
...
//this function will check to see if the node is withing the space
bool checkNode(struct pointNode * curr, GLfloat xMin, GLfloat xMax, GLfloat yMin, GLfloat yMax, GLfloat zMin, GLfloat zMax)
{
	//This function will keep track where the brownian motion collides in an integer number
	//and the number will be used to make the opaque wall.

	//these are the sloped lines of the octogon on the xy plane
	//they will be used to check if they are within the octogan on the xy plane
	...
	GLfloat b; //this is the intercept

	//these check if it's within the box
	if (curr->x < xMin)
	{
...
// Load shaders and use the resulting shader programs
	...
	/////////START AND END MARKER SHADERS//////////
	program[3] = InitShader("vshader30_TwoCubes_FullPipe.glsl", "fshader30_TwoCubes.glsl");
	program[4] = InitShader("vshader30_TwoCubes_FullPipe.glsl", "fshader6_2.glsl");
	program[5] = InitShader("vshader30_TwoCubes_FullPipe.glsl", "fshader30_TwoCubes.glsl");
	program[6] = InitShader("vshader30_TwoCubes_FullPipe.glsl", "fshader6_2.glsl");
	...
	glBindBuffer(GL_ARRAY_BUFFER, buffers[cube_vertices]);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffers[cube_indeces]);
	GLuint vPosition_Cube2 = glGetAttribLocation(program[3], "vPosition");
	...
	glBindBuffer(GL_ARRAY_BUFFER, buffers[cube_vertices]);
	GLuint vPosition_Wire2 = glGetAttribLocation(program[4], "vPosition");
	...
	glBindBuffer(GL_ARRAY_BUFFER, buffers[cube_vertices]);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffers[cube_indeces]);
	GLuint vPosition_Cube3 = glGetAttribLocation(program[5], "vPosition");
	...
	glBindBuffer(GL_ARRAY_BUFFER, buffers[cube_vertices]);
	GLuint vPosition_Wire4 = glGetAttribLocation(program[6], "vPosition");
...
display( void )
{
	...
	/*********************************************************
	*	In this if statement, this will check where the 
	*	collision took place, and render the opaque wall.
	*********************************************************/
	if (bComplete == true)
	{
		if (collisionArea == 1)
		{
			//offset, triangle buffers
			//36, 6 
			glUseProgram(program[0]);
	...
	if (bComplete == true)
	{
		...
		GLuint color_loc2 = glGetUniformLocation(program[6], "color");
		glUniformMatrix4fv(proj_loc, 1, GL_TRUE, projmat);
		glUniformMatrix4fv(model_view_loc, 1, GL_TRUE, modelviewStackTop);
		glBindVertexArray(VAO[1]);
	...
	GLuint color_loc = glGetUniformLocation(program[4], "color");
	glUniformMatrix4fv(proj_loc, 1, GL_TRUE, projmat);
	glUniformMatrix4fv(model_view_loc, 1, GL_TRUE, modelviewStackTop);
	glBindVertexArray(VAO[1]);
	glDrawElements(GL_LINES, 100, GL_UNSIGNED_BYTE, BUFFER_OFFSET(0));

	modelviewStackTop = modelviewStackBottom*Translate(head->x, head->y, head->z)*Scale(0.5 / 20, 0.5 / 20, 0.5 / 20); //mvstack.pop


																													   //Render surface mesh
	glUseProgram(program[3]);
	color_loc = glGetUniformLocation(program[3], "color");
	proj_loc = glGetUniformLocation(program[3], "projection");
	model_view_loc = glGetUniformLocation(program[3], "modelview");
	glUniformMatrix4fv(proj_loc, 1, GL_TRUE, projmat);
	glUniformMatrix4fv(model_view_loc, 1, GL_TRUE, modelviewStackTop);
	...
	case 'Y': {	thetaY += step;
		if (thetaY > 360.0) {
			thetaY -= 360.0;
			  ...
			  break;
	case 'i': case 'I':{ //pause brownian
		bPaused = !bPaused;
		if (!bPaused)
			animate(0);
		break;
	}
	case 's': case 'S': { //start brownian
		bPaused = !bPaused;
		if(!bPaused)
			animate(0);
		break;
